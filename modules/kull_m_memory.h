...
typedef struct _KULL_M_MEMORY_HANDLE {  //内存访问系统的核心句柄结构体，用于统一处理不同类型的内存访问
	KULL_M_MEMORY_TYPE type;
	union {
		PKULL_M_MEMORY_HANDLE_PROCESS pHandleProcess;  //进程
		PKULL_M_MEMORY_HANDLE_FILE pHandleFile;  //文件
		PKULL_M_MEMORY_HANDLE_PROCESS_DMP pHandleProcessDmp; //转储文件
		PKULL_M_MEMORY_HANDLE_KERNEL pHandleDriver;  //驱动
	};
...
typedef struct _KULL_M_MEMORY_ADDRESS {
	LPVOID address;   //内存地址
	PKULL_M_MEMORY_HANDLE hMemory; //内存句柄
